...
for i = 1 : (numSamples)
    ...
    b_omega_dot = sigma_b;
    b_omega(1,i+1) = b_omega(1,i) + b_omega_dot(1,1)*dt;
    b_omega(2,i+1) = b_omega(2,i) + b_omega_dot(2,1)*dt;
    b_omega(3,i+1) = b_omega(3,i) + b_omega_dot(3,1)*dt;
    %disp(b_omega(:,i+1))
    ...
    omega_x = angvel_x(i) - b_omega(1,i+1) + sigma_R(1);
    omega_y = angvel_y(i) - b_omega(2,i+1) + sigma_R(2);
    omega_z = angvel_z(i) - b_omega(3,i+1) + sigma_R(3);
    ...
    attitude_angles(i+1,1) = wrapToPi(attitude_angles(i+1,1));

    pitch_dot = cos(roll)*omega_y - sin(roll)*omega_z;
    attitude_angles(i+1,2) = attitude_angles(i,2) + pitch_dot*dt;
    attitude_angles(i+1,2) = wrapToPi(attitude_angles(i+1,2));
